<div id="comments" class="comments-area block">
    <h3 class="block-title"><span>{{ $article->approvedComments->count() }} Comments</span></h3>
                ...
                @if(!$comment->parent_id)
                    <div class="comment">
                        ...
                        <div class="comment-body">
                            <div class="meta-data">
                                <span class="comment-author">{{ ($comment->isGuest() ? 'Guest user: ' : '') . $comment->commenterName() }}</span>
    ...
    <span>{{ \Carbon\Carbon::parse($comment->created_at)->diffForHumans() }}</span>
                            ...
                            <div class="text-left">
                                <a class="comment-reply" data-toggle="modal" data-target="#commentReplyModal{{ $comment->id }}">Reply</a>
                                @canany('update', $comment)
                                    <a class="comment-edit" data-toggle="modal" data-target="#commentEditModal{{ $comment->id }}" style="margin-right: 1rem ">Edit</a>
                                @endcanany
                                ...
                                @canany('delete', $comment)
                                    ...
                                    <a onclick="event.preventDefault(); document.querySelector('#deleteForm{{ $comment->id }}').submit()" class="comment-delete">Delete</a>
                                @endcanany
                            </div>
                                                ...
                                                <form id="commentForm" role="form" action="{{ route('comments.update', $comment->id) }}" method="POST">
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-12">
                                                            ...
                                                            <textarea class="form-control required-field" name="comment" id="comment" placeholder="Your comment">{{ old('comment', $comment->comment) }}</textarea>
                                                            </div>
                                                        </div><!-- Col end -->
                                                    </div>
